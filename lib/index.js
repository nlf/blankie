...
internals.generatePolicy = function (options, request) {
    ...
    internals.directiveNames.forEach((key) => {
        ...
        if ((key === 'scriptSrc' || key === 'styleSrc') && internals.nonceShouldBeGenerated(options, key)) {
            ...
            const nonce = request.plugins.blankie.nonces[internals.keyToConfigMap[key]];
...
internals.attachNonces = function (request, reply) {
...
internals.addHeaders = function (request, reply) {
    ...
    const contentType = (response.headers || response.output.headers)['content-type'];
