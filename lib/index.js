...
internals.addHeaders = function (request, next) {
    ...
    if (request.route.plugins.blankie === false) {
        return next();
    }

    var options;

    if (request.route.plugins.blankie) {
        options = internals.validateOptions(request.route.plugins.blankie);
    ...
    switch (userAgent.family) {
        case 'Chrome':
            if (userAgent.satisfies('>=14.0.0 <26.0.0')) {
        ...
        case 'Firefox':
            ...
            else if (userAgent.satisfies('>=5.0.0  <24.0.0')) {
                headerName = 'X-Content-Security-Policy';
        ...
        case 'Safari':
            if (userAgent.satisfies('^6.0.0') ||
               (userAgent.satisfies('^5.0.0') && options.oldSafari)) {
                headerName = 'X-WebKit-CSP';
            }
            policy = internals.generatePolicy(options);
